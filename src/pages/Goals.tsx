  ...
  const loadGoals = async () => {
    ...
    try {
      const { data: goalsData, error: goalsError } = await supabase
        ...
        .select(`
          id, type, target_value, start_period, end_period, created_at,
          categories(name)
        `)
        .eq('user_id', user.id)
    ...
    <div className="space-y-4">
      {goalsList.length === 0 ? (
        <div className="text-center py-8">
          ...
          <Button style={{ background: type === 'receita' ? 'var(--goal-gradient)' : 'var(--expense-gradient)' }}>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button style={{ background: 'var(--goal-gradient)' }}>
          <Plus className="h-4 w-4 mr-2" />
          Nova Meta
        </Button>
      </div>
